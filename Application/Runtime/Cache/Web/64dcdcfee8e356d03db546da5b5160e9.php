			...
			<div class="webBox container">
				...
				<ul class="listUl">
					<?php if(is_array($data["web"])): $i = 0; $__LIST__ = $data["web"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$web): $mod = ($i % 2 );++$i;?><li class="listLi">
							<div class="listTitle"><a href="/web/detail/article/channelid/2/id/<?php echo ($web["id"]); ?>"><?php echo ($web["title"]); ?></a></div>
				...
				<ul class="categoryUl">
					...
					<?php if(is_array($categorys)): $i = 0; $__LIST__ = $categorys;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><li><a href="/web/Web/webbycategory/category/<?php echo ($cate["id"]); ?>"><?php echo ($cate["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul>
			</div>
		</div>
		<!--分页-->
